/*
 * Ultrasonic ranger/Servo driver main source file
 * This file will run the ultrasonic sensor and servo
 * 
 * Functionality:
...
// PWM defines
#define PWM_PERIOD_NS 20000000  // 20ms (50Hz) Prescaler is 10k
#define SERVO_MIN_PULSE_NS 400000    // Try 400 µs instead of 500
#define SERVO_MAX_PULSE_NS 2700000   // Try 2.7 ms instead of 2.5
#define PWM_DUTY_CYCLE_USEC (PWM_PERIOD_NS / 2) // 50% duty cycle
    ...
    while (1) {
        ...
        if (dist1 > 0) {
            if (dist1 <= WARNING_DIST) {
                ...
                ring_buf_put(&ultra_ring_buf, (uint8_t *)&dist1, sizeof(dist1));
                // broadcast_distance(dist1);
